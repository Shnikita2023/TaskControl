...
from tests.test_data import parties_answer
from tests.test_data import updated_parties
from tests.test_data import updated_parties_filter
...
class TestParty:
    ...
    async def test_update_party(self, async_client: AsyncClient) -> None:
        response_party: Response = await async_client.patch(url="/api/v1/parties/1", json=updated_parties)
        assert response_party.status_code == 200
    ...
    async def test_get_party_by_filter(self, async_client: AsyncClient) -> None:
        response_party: Response = await async_client.get(
            url="/api/v1/parties/?value_party=22222&" "offset=0&limit=1&name_party=party_number"
        )
        assert response_party.status_code == 200
