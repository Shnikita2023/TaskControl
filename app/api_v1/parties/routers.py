...
@router_party.post(path="/",
                   summary="Создание сменного задание",
...
@router_party.get(path="/{party_id}",
                  summary="Получение сменного задание",
                  response_model=PartyShow)
async def get_party(party_id: int,
                    session: AsyncSession = Depends(get_async_session)) -> PartyShow:
    return await party_service.get_party(session=session, party_id=party_id)


@router_party.patch(path="/{party_id}",
                    summary="Обновление сменного задание",
                    response_model=PartyUpdate)
async def update_party(party_id: int,
                       new_party: PartyUpdate,
                       session: AsyncSession = Depends(get_async_session)) -> PartyUpdate:
    return await party_service.update_party(session=session, new_party=new_party, party_id=party_id)


@router_party.get(path="/",
                  summary="Получение сменных заданий по различным фильтрам")
async def get_parties_by_filter(value_party: str = Depends(check_type_value_party),
                                offset: int = Query(ge=0, default=0),
                                limit: int = Query(ge=1, default=1),
                                name_party: str = "party_number",
                                session: AsyncSession = Depends(get_async_session)) -> list[dict[str, Any]]:
    return await party_service.get_parties_by_filter(session=session,
