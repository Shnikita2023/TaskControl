...
@router_product.get(path="/", summary="Проверка агрегации продукции к партии")
async def check_aggregation_product(
    code_product: str,
    party_id: int = Query(ge=1),
    session: AsyncSession = Depends(get_async_session),
) -> dict:
    return await product_service.check_aggregation_product(
        session=session, code_product=code_product, party_id=party_id
    )
